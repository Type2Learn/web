...
try {
  ...
  const tracks = await page.evaluate(() => window.__urduNarrationTracks.slice(0, 3));
  assert.equal(tracks[0].includes('/01-adhd/urdu-pk/title-ava.mp3'), true, 'Urdu narration must begin with the Urdu title recording.');
  assert.equal(tracks[1].includes('/01-adhd/urdu-pk/section-1-heading-ava.mp3'), true, 'The Urdu heading must follow the Urdu title.');
  assert.equal(tracks[2].includes('/01-adhd/urdu-pk/section-1-answer-ava.mp3'), true, 'The Urdu explanation must follow its Urdu heading.');
