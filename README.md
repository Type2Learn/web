...
| **Understanding checks** | Delivers one question at a time from a reviewed or authored reserve. A deterministic objective-evidence monitor guides a precise review route; Mini may recognise a safe paraphrase, but never decides alone. A learner may separately opt in to retain their submitted own-word assessment evidence for up to 90 days so later attempts at the same objective can be compared. | Expose correct answers, visible percentages, speed scores, a learner profile, or retain any answer without that second explicit choice. |
...
| Guided course and local guest progress | Yes | Runs without an AI provider; signed-in sync stores a bounded resume marker only—never typed drafts, chat text, transcripts, or selected assessment answers. A signed-in learner can separately opt in to retain submitted open assessment evidence, with contact details redacted and a 90-day expiry. |
...
- `/admin/` bootstraps the first platform administrator with a one-time server-side secret, then manages role codes, source review, a structured bilingual course form with **Build & validate course**, direct reviewed-Markdown import, PDF/PPTX source-to-course conversion, AI drafts, narration, backup receipts, release approval, audit history, and a non-destructive learner preview. Administrators can also deliberately remove a selected draft or published course version from the workspace/catalogue after typing `DELETE`; its source, immutable backup receipt, audit trail, and learner records are retained as separate protected records.
- `/teacher/` and `/institute/` accept **theory** course source material, show review status, create scoped learner invites, maintain private rosters, and distribute only approved courses to their organisation or assignment list. The `/teacher/` handover accepts `.pdf`, `.pptx`, `.md`, `.markdown`, `.txt`, and `.csv` sources and captures a private learning goal, learner context, and source language so review starts with the educator’s intent. Coding, project, and other course types are visibly locked and rejected by the API until their learning engines exist.
