...
const bindAuthoring = () => {
    ...
    try {
      ...
      if (conversionOutput && conversion) {
        if (conversion.state === 'running') {
          ...
          const conversionNotice = conversion.notice
            ? `${conversion.notice}\n\n`
            : '';
          const summary = {
          ...
          conversionOutput.textContent = `${conversionNotice}Saved conversion draft — still requires human review:\n${JSON.stringify(summary, null, 2)}`;
  ...
  // AUDIT → EDITABLE REVIEW: the safe preview intentionally contains no
  // answer keys, source upload, or publishing control. This route opens the
  // existing authenticated one-module-at-a-time editor, retaining that
  // boundary while making the preview's next action useful.
  const openEditableCourseReview = async () => {
    const { courseId, version } = selectedCourse();
    if (!courseId || !version) throw new Error('Choose a validated course before opening its editable review.');
    ...
    status('Editable course review is open. Use a pencil to edit one module, then save and revalidate the complete course.', 'success');
  };
  ...
  $('[data-open-course-review]')?.addEventListener('click', async () => {
    try { await openEditableCourseReview(); } catch (error) { status(error.message, 'warning'); }
  });
  $('[data-learner-preview]')?.addEventListener('click', async (event) => {
    if (!event.target.closest('[data-open-editable-course-review]')) return;
    try { await openEditableCourseReview(); } catch (error) { status(error.message, 'warning'); }
  });
