...
export const companionDockMarkup = ({ language, escapeHtml, draft = '', canSpeak = false, channel = 'text', listening = false }) => {
  ...
  return '<div class="course-companion-dock" data-companion-dock><label><span class="course-visually-hidden">' + escapeHtml(copy(language, 'Message your learning partner', 'اپنے سیکھنے کے ساتھی کو پیغام دیں')) + '</span><textarea rows="1" maxlength="900" data-companion-input placeholder="' + escapeHtml(copy(language, voiceEnabled ? 'Say something, then review it before sending…' : 'Message your learning partner…', voiceEnabled ? 'کچھ کہیں، پھر بھیجنے سے پہلے اسے دیکھیں…' : 'اپنے سیکھنے کے ساتھی کو پیغام دیں…')) + '">' + escapeHtml(draft) + '</textarea></label><div>'
    ...
    + '<button type="button" class="course-primary-button" data-action="companion-send">' + escapeHtml(copy(language, 'Send', 'بھیجیں')) + '</button></div></div>';
};
